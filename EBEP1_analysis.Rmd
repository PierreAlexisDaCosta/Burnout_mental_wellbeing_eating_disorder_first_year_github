...
# Rename columns with English names (if available)
...
# Define response order for typical WEMWBS categories (adjust based on your actual coding)
...
# Add labels and highlight specific questions
WE_Reponse_EBE_P1_final_summary <- WE_Reponse_EBE_P1_final_summary %>%
  mutate(bold_label = ifelse(Question %in% bold_questions, TRUE, FALSE),
         percent_label = ifelse(percent > 0.02, paste0(round(percent*100), "%"), ""))

# Color palette for WEMWBS
...
SCOFF_Reponse_EBE_P1_final_for_score <- SCOFF_Reponse_EBE_P1_final %>%
  mutate(across(everything(), ~ case_when(
    ...
    TRUE ~ NA_real_  # or NA_integer_ if you want integers
  )))

# 4. Calculate total score per individual
...
SCOFF_summary <- SCOFF_Reponse_EBE_P1_final_score %>%
  ...
  mutate(percent = n / sum(n),
         ...
         SCOFF_num = as.numeric(as.character(SCOFF_total)) # Convert to numeric
  ) %>%
  mutate(SCOFF_total = factor(SCOFF_total, levels = as.character(0:5)))

# Compute cumulative height at cutoff SCOFF_num == 2
...
SCOFF_score_ditrib <-
  ...
  scale_fill_gradientn(
    colors = c("#FFC0CB", "#DA70D6", "#8A2BE2"),  # gradient pink -> purple
    breaks = 0:5,
    labels = 0:5,
    limits = c(0,5),
...
# 6. Rename columns with English names (if available)
...
# Filter correspondence for MBI-SS
MBI_correspondance <- Correspondance_EBEP1_2025 %>%
  filter(Echelle == "French_MBI_SS")

# Select only MBI columns in the responses
...
# Remove "[MBI-SS]" from English names in the correspondence
...
MBI_Reponse_EBE_P1_final_for_score <- MBI_Reponse_EBE_P1_final_for_score %>%
  mutate(
    # Emotional Exhaustion: sum of 5 items
    ...
    # Academic Efficacy: sum of 6 items
...
# Select scores and items for final dataset
...
# Long format + compute proportions
...
# Calculate statistics + number and % < 40
stats_WE <- All_scores %>%
  summarise(
...
# Descriptive statistics for Cynicism
stats_CY <- All_scores %>%
  summarise(
...
# Counts + % per Cynicism_Level (existing column)
...
# Descriptive statistics Academic_Efficacy
stats_AE <- All_scores %>%
  summarise(
...
# Counts + % per Academic_Efficacy_Level (existing column)
...
# Add stars to the graph
# Add stars to the graph, slightly higher up and in gray
n <- ncol(cor_matrix)
for (i in 1:(n-1)) {
  for (j in (i+1):n) {
    star <- stars_mat[i, j]
    if (star != "") {
      x_pos <- j
      y_pos <- n - i + 1
      # Vertical offset: +0.3 to raise slightly
...
make_upset_plot <- function(data, prefix, keep_order = FALSE) {
  # 1. Select columns and recode to binary
  df_binary <- data %>%
...
# Single-variable Baccalaureate field distribution
bacc_single <- Reponse_EBE_P1_final %>%
  filter(!is.na(Baccalaureate_Field)) %>%
  count(Baccalaureate_Field) %>%
  mutate(freq = n / sum(n) * 100)

# Color palette (soft pastel colors for 4 categories)
...
# Recoding function that keeps values not found
